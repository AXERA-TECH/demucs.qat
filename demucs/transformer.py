...
def create_2d_sin_embedding(d_model, height, width, device="cpu", max_period=10000):
    ...
    # div_term = torch.exp(
    #     torch.arange(0.0, d_model, 2) * torch.tensor(-math.log(max_period) / d_model).to(torch.arange(0.0, d_model, 2))
    # )
    div_term = torch.exp(
        torch.arange(0.0, d_model, 2).to('cuda') * torch.tensor(-math.log(max_period) / d_model).to('cuda')
    )
    pos_w = torch.arange(0.0, width).unsqueeze(1).to('cuda')
    pos_h = torch.arange(0.0, height).unsqueeze(1).to('cuda')
    pe[0:d_model:2, :, :] = (
...
class MyTransformerEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        if self.norm_first:
            x = x + self.gamma_1(
                self.sa_block(self.norm1(x), src_mask, src_key_padding_mask)
            )
            x = x + self.gamma_2(self.ff_block(self.norm2(x)))

            if self.norm_out:
                x = self.norm_out(x)
        else:
            x = self.norm1(
                x + self.gamma_1(self.sa_block(x, src_mask, src_key_padding_mask))
            )
            x = self.norm2(x + self.gamma_2(self.ff_block(x)))
...
class CrossTransformerEncoder(nn.Module):
    ...
    def forward(self, x, xt):
        ...
        x = self.norm_in(x)
        x = x + torch.tensor(self.weight_pos_embed).to(x) * pos_emb_2d
        ...
        xt = xt + torch.tensor(self.weight_pos_embed).to(x) * pos_emb
    ...
    def _get_pos_embedding(self, T, B, C, device):
        if self.emb == "sin":
            shift = torch.rand(self.sin_random_shift + 1).to(device)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if self.auto_sparsity:
            ...
            # x = F.scaled_dot_product_attention(q, k, v, attn_mask, dropout=self.attn_drop)
