...
class HTDemucs(nn.Module):
    ...
    def _spec(self, x):
        ...
        pad = hl // 2 * 3
        # x = F.pad(x, (pad, pad + le * hl - x.shape[-1]), mode="reflect", value=0.0)
    ...
    #     bs,c1,c2,h,w = z.size()
    #     x1 = torch.zeros(bs*c1*c2*(h+1)*w , dtype=z.dtype).reshape(bs,c1,c2,h+1,w).to(z)
    #     x1 = x1.clone() # clone the tensor
    #     x1[:,:,:,0:h,:] = z
    #     x2 = torch.zeros(bs*c1*c2*(h+1)*(w+4) , dtype=z.dtype).reshape(bs,c1,c2,h+1,(w+4)).to(z)
    #     x2 = x2.clone()
    #     x2[:,:,:,:,2:w+2] = x1
    #     # z = x2.clone()
    #     # z = F.pad(z, (0, 0, 0, 1)) #报错No decompositions registered for the complex-valued input
    #     # z = F.pad(z, (2, 2))
    #     pad = hl // 2 * 3
    #     le = hl * int(math.ceil(length / hl)) + 2 * pad
    #     x = ispectro(x2, hl, length=le)
    #     x = x[..., pad: pad + length]
    #     return x
    ...
    def forward_for_export(self, mix, mag):
        ...
        for idx, encode in enumerate(self.encoder):
            ...
            if idx == 0 and self.freq_emb is not None:
                ...
                frs = torch.arange(x.shape[-2], device=x.device)
                emb = self.freq_emb(frs).t()[None, :, :, None].expand_as(x)
                x = x + torch.tensor(self.freq_emb_scale) * emb
